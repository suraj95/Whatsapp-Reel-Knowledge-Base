...
async def node_resolve_destination(state: TripPlannerState) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {"days": days, "destination_label": label or dest_q, "origin_label": olab, "debug_notes": notes}
    if lat is not None:
...
async def node_gather_context(state: TripPlannerState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if index is not None and query_embedding is not None:
        try:
            ...
            # Match search/recommendation: do not pull extra reels just for multi-day layout.
    ...
    async def _weather():
        if lat is None or lon is None:
            return None
        raw = await asyncio.to_thread(open_meteo.fetch_forecast_daily, float(lat), float(lon), days=max(days, 3))
...
async def node_compose(state: TripPlannerState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if client is not None:
        try:
            ...
            logger.exception("trip compose llm: %s", ex)
            narrative = ""
